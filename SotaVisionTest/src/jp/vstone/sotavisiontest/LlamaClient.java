...
/**
 * HTTP client for communicating with a locally running Ollama LLaMA server.
 * Uses streaming NDJSON to minimize perceived latency.
 *
 * Connects to Ollama running on the laptop (default: http://localhost:11434).
 * When running on Sota, set the URL to the laptop's IP address.
 *
 * No external dependencies — pure java.net + java.io (Java 1.8 compatible).
 */
public class LlamaClient {
    ...
    /** Default constructor: localhost, llama3.2:3b, 60 tokens, 120s timeout. */
    public LlamaClient() {
    ...
    /**
     * Send a single-turn message (system prompt + user message) to the LLM.
     */
     ...
     * @param history       List of raw JSON message objects: {"role":"...","content":"..."}
     */
    ...
    // ----------------------------------------------------------------
    // Core HTTP call — streaming NDJSON to Ollama /api/chat
    // ----------------------------------------------------------------
    ...
    private LLMResult sendToLLM(String messagesJson) {
        ...
        log("Response received: TTFT=" + String.format("%.0f", ttftMs)
            + "ms, total=" + String.format("%.0f", totalMs) + "ms, "
            + evalTokens + " tokens, " + String.format("%.1f", tps) + " tok/s");
    ...
    // ----------------------------------------------------------------
    // JSON helpers (manual, no Gson)
    // ----------------------------------------------------------------

    /** Build a JSON message: {"role":"...","content":"..."} */
